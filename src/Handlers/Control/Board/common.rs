...
impl Pawn {
  pub fn new(color: Colors) -> Pieces {
    return Pieces::Pawn(Pawn {value:1,color: color.clone(), icon: match color{ Colors::White => String::from("P"), Colors::Black => String::from("p")}});
  }
...
impl Rook {
  pub fn new(color: Colors) -> Pieces {
    return Pieces::Rook(Rook {value: 5,color: color.clone(),icon: match color{ Colors::White => String::from("R"), Colors::Black => String::from("r")}});
  }
...
impl Bishop {
  pub fn new(color: Colors) -> Pieces {
    return Pieces::Bishop(Bishop {value: 3,color: color.clone(), icon: match color{ Colors::White => String::from("B"), Colors::Black => String::from("b")}});
  }
  pub fn check_move(&mut self, code: Vec<&str>, entity: Pieces) -> movestate {
    match entity.clone() {
      Pieces::Pawn(e) => {if e.color==self.color {return movestate::Illegal}},
    ...
    if opos + (oindex as i32) != npos + (nindex as i32) && opos - (oindex as i32) != npos - (nindex as i32) {
      return movestate::Illegal
    }
    return movestate::Legal
...
impl Queen {
  pub fn new(color: Colors) -> Pieces {
    return Pieces::Queen(Queen {value: 8,color: color.clone(), icon: match color{ Colors::White => String::from("Q"), Colors::Black => String::from("q")}});
  }
  pub fn check_move(&mut self, code: Vec<&str>, entity: Pieces) -> movestate {
    match entity.clone() {
      Pieces::Pawn(e) => {if e.color==self.color {return movestate::Illegal}},
    ...
    if (opos + (oindex as i32) != npos + (nindex as i32) && opos - (oindex as i32) != npos - (nindex as i32)) && (opos!=npos && nindex!=oindex) {
      return movestate::Illegal
    }
...
impl King {
  ...
  pub fn check_move(&mut self, code: Vec<&str>, entity: Pieces) -> movestate {
    ...
    if !(((nindex as i32-oindex as i32).abs() < 2) && ((npos-opos).abs() < 2)) {
      return movestate::Illegal
    }
    return movestate::Legal
  }
